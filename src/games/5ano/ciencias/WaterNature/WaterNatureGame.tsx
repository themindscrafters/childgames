...
import { getQuestionsForDifficulty, getConceptByName } from './waterNatureData';
import type { NatureQuestion } from './waterNatureData';

const TOTAL_ROUNDS = 10;

export function WaterNatureGame() {
  ...
  const generateRound = useCallback(() => {
    ...
    setTimeout(() => setConceptVisible(true), 100);
  }, [difficultyManager]);
  ...
  const handleSelect = useCallback(
    (option: string) => {
      ...
      if (option === currentQuestion.correct) {
        ...
        if (currentQuestion.conceptRelated) {
          const concept = getConceptByName(currentQuestion.conceptRelated);
          if (concept) {
            setCurrentFunFact(concept.funFact);
            setShowFunFact(true);
          }
        }
  ...
  const relatedConcept = currentQuestion?.conceptRelated
    ? getConceptByName(currentQuestion.conceptRelated)
    : null;
    ...
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        <div style={styles.titleRow}>
          ...
          <span style={styles.title}>Água e Natureza</span>
        </div>
      ...
      {/* Concept display — only revealed after correct answer */}
      {feedback === 'correct' && relatedConcept && (
        <div
          style={{
            ...styles.conceptArea,
            animation: 'conceptReveal 0.5s cubic-bezier(0.34, 1.56, 0.64, 1)',
          }}
        >
          <div style={styles.conceptEmoji}>{relatedConcept.emoji}</div>
          <div style={styles.conceptName}>{relatedConcept.name}</div>
          <div style={styles.conceptCategory}>{relatedConcept.category}</div>
        </div>
      )}
      ...
      {feedback !== 'correct' && (
        <div
          style={{
            ...styles.conceptArea,
            transform: conceptVisible ? 'scale(1)' : 'scale(0)',
...
const styles: Record<string, React.CSSProperties> = {
  ...
  bgOverlay: {
    position: 'absolute',
    inset: 0,
    background: 'radial-gradient(circle at 20% 30%, rgba(255,255,255,0.4) 0%, transparent 50%), radial-gradient(circle at 80% 70%, rgba(255,255,255,0.3) 0%, transparent 40%)',
    pointerEvents: 'none',
    zIndex: 0,
  },
  header: {
